...
void main() {
  ...
  testWidgets('Design testing of home page', (tester) async {
    ...
    // Find the first ElevatedButton containing the FontAwesome nurse icon
    final nurseButtonFinder = find.byIcon(FontAwesomeIcons.userNurse).first;
    expect(nurseButtonFinder, findsOneWidget); // Ensure the button is found

    // Tap the nurse button to show the alert dialog
    await tester.tap(nurseButtonFinder);

    // Wait for the dialog to appear (add a little extra time if needed)
    await tester.pumpAndSettle(Duration(seconds: 2)); // Wait for dialog

    // Verify that the alert dialog opened (ensure it exists)
    final nurseDialogFinder = find.byType(AlertDialog);
    expect(nurseDialogFinder, findsOneWidget); // Ensure the dialog is found

    // You may also want to tap on the dialog close button as part of the test:
    final nurseDialogCloseButton = find.byIcon(Icons.close);
    expect(nurseDialogCloseButton, findsOneWidget);
    await tester.tap(nurseDialogCloseButton);
    await tester.pumpAndSettle(); // Wait for the dialog to close
  });
}
